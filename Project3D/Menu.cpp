...
void Menu::activateCurrentItem() {
	if (menuItems.at(selectedIndex).funcargs != nullptr)
...
// Static builders
Menu Menu::ofMainMenu(void(*func1)(char* MenuType)) {
	Menu menu;
	// Add menuitems
	menu.addMenuItem(new MenuItem{ 0, "Play Game", func1, "MainMenu" });
...
Menu Menu::ofHelpMenu(void(*goback)(char* MenuType), void(*func0)(), void(*func1)(), void(*func2)()) {
	...
	menu.addMenuItem(new MenuItem{ 0, "Objects" });
	menu.addMenuItem(new MenuItem{ 1, "Powers Ups"});
	menu.addMenuItem(new MenuItem{ 2, "Extra" });
...
Menu Menu::ofSettingsMenu(void(*goback)(char* MenuType), void(*func0)(), void(*func1)(), void(*func2)()) {
	...
	menu.addMenuItem(new MenuItem{ 0, "Sound/SFX" });
	menu.addMenuItem(new MenuItem{ 1, "Music" });
	menu.addMenuItem(new MenuItem{ 2, "Sensitivity"});
...
Menu Menu::ofThemeMenu(void(*goback)(char* MenuType), void(*func0)(), char* func0args) {
	Menu menu;

	menu.addMenuItem(new MenuItem{ 0, "Map" });
	menu.addMenuItem(new MenuItem{ 1, "Player" });
	menu.addMenuItem(new MenuItem{ 2, "Enemy" });
